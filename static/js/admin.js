...
function togglePassword() {
   const passwordField = document.querySelector("#new-pass");
   const toggleButton = document.querySelector(".eye-button");
...
function validateDni() {
    ...
    if (!nifRexp.test(str) && !nieRexp.test(str)) {
        alert('El documento de identidad no es correcto, modifique su contenido para continuar.')
        return false;
    }
...
function validateEmail(){
	var emailField = document.getElementById('new-email');
	var validEmail =  /^\w+([.-_+]?\w+)*@\w+([.-]?\w+)*(\.\w{2,10})+$/;

	if( validEmail.test(emailField.value)){
		return true;
	}else{
		alert('El email no es correcto, modifique su contenido para continuar.');
		return false;
	}
}

function validateStudent() {
    if (validateDni() && validateEmail()) {
        return true;
    }
    return false;
}
